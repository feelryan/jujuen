...
MAX_RETRIES_PER_TOPIC = 5
DEFAULT_TOTAL_QUESTIONS_PER_TOPIC = 12

BASE_DIR = Path(__file__).resolve().parent
PROJECT_ID_FILE = BASE_DIR / "PROJECT_ID.txt"
OUTPUT_DIR = BASE_DIR / "output"
PROMPT_QUESTIONS_FILE = BASE_DIR / "prompt-handbook-questions.md"
...
def send_to_vertex_for_questions(
    client: genai.Client,
    prompt: str,
) -> str | None:
...
def main(
    ...
    for skill_id in skill_ids:
        print(f"\n===== Processing handbook questions for skill '{skill_id}' =====")
        ...
        if not handbook_obj:
            print("  ! Invalid handbook.json, skipping this skill.")
            continue

        items = handbook_obj.get("items") or []
        if not isinstance(items, list) or not items:
            print("  ! handbook.json.items 為空，無主題可產生題目。")
            continue

        # 只針對主 handbook 章節（排除 id 以 -quiz 結尾者）產生題目
        ...
        handbook_topics_dir = skill_dir / "handbook"
        ...
        for it in main_topics:
            ...
            material_path = handbook_topics_dir / f"{topic_id}.md"
            if not material_path.exists():
                print(f"  - [Skip] Handbook material not found for topic {topic_id}: {material_path}")
                continue

            questions_path = handbook_topics_dir / f"{topic_id}.json"
            meta_path = handbook_topics_dir / f"{topic_id}.questions-meta.json"
            ...
            full_prompt = (
                prompt
                + "\n\n---\n\n"  # 分隔指示與手冊內容
                + "以下是完整的 Handbook 章節 Markdown 內容，請依此出題（避免重複既有題目）：\n\n"
                + material_text
    ...
    print("\n===== All skills and handbook topics processed. Question generation complete. =====")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Generate question JSON files for each handbook topic under workflow/skills/output/{skill}/handbook/ "
            "based on handbook Markdown. If --skill-id is omitted, all skills with handbook.json under output/ will be processed."
        )
    )
    parser.add_argument(
        "--skill-id",
        required=False,
        help="Skill id (directory name under output/) for which to generate handbook questions. If omitted, process all skills with handbook.json.",
    )
    parser.add_argument(
        "--topic-id",
        required=False,
        help="Only generate questions for the specified handbook topic id.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
