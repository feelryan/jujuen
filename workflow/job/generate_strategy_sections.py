...
PROMPT_STRATEGY_FILE = BASE_DIR / "prompt-strategy-category.md"
...
def main(resume_path: Path, jd_path: Path, dir_name: str | None = None, force: bool = False) -> None:
    # 讀取 PROJECT_ID
    ...
    try:
        prompt_template = load_text_file(PROMPT_STRATEGY_FILE)
    ...
    if strategy_path.exists() and not force:
        print(f"strategy.json already exists: {strategy_path}")
        print("Use --force to overwrite.")
        return

    print(f"Generating best-strategy sections for resume='{resume_base}' jd='{jd_base}'...")
    ...
    if result_obj is None:
        print("  ! 無法成功取得有效的 Strategy Category JSON，請稍後重試或檢查 prompt。")
        return

    # 重新包裝成我們要的 Strategy Category 結構，並補上 pointTo 與 quiz item
    ...
    for idx, item in enumerate(items, start=1):
        ...
        # 主策略節點：指向對應的 Markdown 策略說明
        main_point_to = {
            "type": "document",
            "data": "github/md",
            "dataLocation": f"{GITHUB_BASE}/{dir_name}/strategy/{topic_id}.md",
        }
        ...
        # 對應的 quiz 子節點：指向該策略主題的題庫 JSON
        quiz_point_to = {
            "type": "document",
            "data": "github/json",
            "dataLocation": f"{GITHUB_BASE}/{dir_name}/strategy/{topic_id}.json",
    ...
    strategy_category = {
        "id": "strategy",
        "title": f"Best Strategy for {resume_base} vs {jd_base}",
        "items": normalized_items,
    }

    save_json(strategy_path, strategy_category)

    print("\nDone. strategy.json generated at:")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Generate best-strategy Category JSON (strategy.json) based on resume and JD PDFs. "
            "Output is written under workflow/job/output/{dir_name}/strategy.json."
        )
    )
    parser.add_argument(
        "--resume",
    ...
    parser.add_argument(
        ...
        required=False,
        help=(
            "Optional output directory name under output/. "
            "If omitted, defaults to '{resume_base}_{jd_base}'."
        ),
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Overwrite existing strategy.json if it already exists.",
    )
