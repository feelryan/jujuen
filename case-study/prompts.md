 ...
 **考題設計準則 (High-Fidelity Guidelines):**
 1. **情境長度 (Scenario Depth):** 每題題幹需包含複雜的情境描述（建議 >500 字元），必須結合案例中的業務目標（如：減維運負擔、維持合規）與技術現狀。
 2. **選項結構 (Option Complexity):** 每個選項必須是完整的行動建議（建議 >120 字元），而非簡短名詞。
