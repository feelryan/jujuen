...
When your configuration contains a resource `resource "google_compute_instance" "foo"`, Terraform uses this map to know that instance `i-abcd1234` is represented by that resource.
...
[Remote state](https://www.terraform.io/docs/state/remote/index.html) is the recommended solution to this problem. With a fully featured state backend, Terraform can use remote locking as a measure to avoid multiple different users accidentally running Terraform at the same time; this ensures that each Terraform run begins with the most recent updated state.

## State locking

If supported by your [backend](https://www.terraform.io/docs/backends/index.html), Terraform will lock your state for all operations that could write state. This prevents others from acquiring the lock and potentially corrupting your state.
...
Not all backends support locking. View the list of [backend types](https://www.terraform.io/docs/backends/types/index.html) for details on whether a backend supports locking.
...
6.  Authorize the plugin as instructed. If a project is not automatically selected, click **Select a Google Cloud Project** and choose `qwiklabs-gcp-01-6f9505a31fc0`.

7.  Verify that your Google Cloud project (`qwiklabs-gcp-01-6f9505a31fc0`) displays in the Cloud Code status message in the status bar.
...
3.  Copy the following Cloud Storage bucket resource code into your `main.tf` configuration file.
    ...
    ```hcl
    provider "google" {
      project = "qwiklabs-gcp-01-6f9505a31fc0" # REPLACE WITH YOUR PROJECT ID
      region  = "us-west1"
    }

    resource "google_storage_bucket" "test-bucket-for-state" {
      name        = "qwiklabs-gcp-01-6f9505a31fc0" # REPLACE WITH YOUR PROJECT ID
      location    = "US"
...
4.  Click **File > Save** but do not close the file.
    ...
    You can learn more about Cloud Storage resources from the [Terraform documentation](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/storage_bucket).

5.  With the `main.tf` file open and Gemini Code Assist enabled in the IDE, notice the presence of the Sparkle icon in the upper-right corner of the editor.
...
7.  To ask Gemini Code Assist to help you edit the `main.tf` configuration file, paste the following prompt into the Gemini Code Assist inline text field that opens from the toolbar.
    ...
    * In the Google provider block, set the project to qwiklabs-gcp-01-6f9505a31fc0.
    * In the "test-bucket-for-state" resource block, update the name of the bucket to qwiklabs-gcp-01-6f9505a31fc0.
    ```
...
9.  When prompted in the **Gemini Diff** view, click **Accept**.
    ...
    ```hcl
    provider "google" {
      project = "qwiklabs-gcp-01-6f9505a31fc0"
      region  = "us-west1"
    }

    resource "google_storage_bucket" "test-bucket-for-state" {
      name        = "qwiklabs-gcp-01-6f9505a31fc0"
      location    = "US"
      uniform_bucket_level_access = true
    }
    ```
...
2.  To change the existing local backend configuration, copy the following configuration into your file, replacing the `local` backend:
    ...
    terraform {
      backend "gcs" {
        bucket  = "qwiklabs-gcp-01-6f9505a31fc0"
...
6.  Run the following command to examine the updates:
    ...
    [Check my progress] (Checked) Assessment Completed!
...
3.  With the `main.tf` file open, notice the presence of the Sparkle icon in the upper-right corner of the editor.
...
5.  To let Gemini Code Assist help you modify the `main.tf` configuration file, paste the following prompt into the Gemini Code Assist inline text field that opens from the toolbar.
    ...
    ```
    When you delete a bucket, this boolean option is used to [delete all contained objects](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/storage_bucket#force_destroy). If you try to delete a bucket that contains objects, Terraform fails that run.
...
[Diagram of Terraform Workflow: Configuration -> Write Config -> Plan -> Apply -> State & Infrastructure]
...
However, you may need to manage infrastructure that wasn't created by Terraform. [Terraform import](https://www.terraform.io/docs/cli/import/index.html) solves this problem by loading supported resources into your Terraform workspace's state.
...
[Diagram of Terraform Import Workflow: Existing Infra -> Import -> Write Config -> Plan -> Apply]
...
3.  Update the provider version in the `terraform.tf` file:
    ```bash
...
4.  Run the following command to initialize your Terraform workspace:
    ```bash
...
5.  In the Cloud Shell Editor, navigate to `learn-terraform-import/main.tf`.

6.  Find the `provider "docker"` resource and **comment out or delete** the `host` argument:
...
7.  Next, navigate to `learn-terraform-import/docker.tf`.

8.  Under the commented-out code, define an empty `docker_container` resource in your `docker.tf` file, which represents a Docker container with the Terraform resource ID `docker_container.web`:

    ```hcl
    resource "docker_container" "web" {}
    ```

9.  Find the name of the container you want to import—in this case, the container you created in the previous step:
    ```bash
    docker ps
    ```
...
10. Run the following `terraform import` command to attach the existing Docker container to the `docker_container.web` resource you just created. Terraform import requires this Terraform resource ID and the full Docker container ID. The command `docker inspect -f {{.ID}} hashicorp-learn` returns the full SHA256 container ID:
    ...
    **Note:** The ID accepted by `terraform import` varies by resource type and is documented in the provider documentation for any resource that can be imported to Terraform. For this example, consult the [Docker provider documentation](https://registry.terraform.io/providers/kreuzwerker/docker/latest/docs/resources/container).

11. Run the following command to verify that the container has been imported into your Terraform state:
    ```bash
    terraform show
    ```
...
5.  You can now selectively remove these optional attributes. **Remove** all of these attributes, keeping only the required attributes: `image`, `name`, and `ports`. After removing these optional attributes, your configuration should match the following.
    ...
    **Output:**
    ```hcl
    resource "docker_container" "web" {
      image = "sha256:87a94228f133e2da99cb16d653cd1373c5b4e0689956386c1c12b66"
...
3.  Run the following command to create an image resource in state. Enter **yes** at the prompt to confirm.
    ...
    terraform apply
    ```
    Now that Terraform has created a resource for the image, you can reference it in your container configuration. You decide to ask Gemini Code Assist to help you do this.

4.  With the `docker.tf` file open, notice the presence of the Sparkle icon in the upper-right corner of the editor.
...
Now that Terraform manages the Docker container, you use Terraform to change the configuration.
...
Terraform import doesn't detect default attributes that don't need to be set in your configuration.
